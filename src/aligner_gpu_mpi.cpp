...
#include "aligner_gpu_presearch_thread_mpi.h"
...
void AlignerGpuMPI::Search(QueryResource &query_resource ,DatabaseType &database,
	...
	for (Queries queries(queries_is, queries_parameters);
		...
		vector<vector<AlignerGpuMPI::PresearchedResult> > presearch_results_list(
				queries.GetNumberOfSequences());
		//vector<vector<AlignerGpuMPI::Result> > 
...
void AlignerGpuMPI::Presearch(Queries &queries, DatabaseType &database,
	...
	for (size_t i = 0; i < number_presearch_threads; ++i) {
		AlignerGpuPresearchThreadMPI t;
		AlignerGpuPresearchThreadMPI::ThreadParameters p;
		p.thread_id = i;
		...
		threads.create_thread(
				boost::bind(&AlignerGpuPresearchThreadMPI::Run, t, p));
	}
	if (database_preload) {
		AlignerGpuPresearchThreadMPI t;
		AlignerGpuPresearchThreadMPI::ThreadParameters p;
...
void AlignerGpuMPI::SetQueriesData(Queries &queries,
...
void AlignerGpuMPI::BuildResults(Queries &queries, DatabaseType &database,
		AligningParameters &parameters,
